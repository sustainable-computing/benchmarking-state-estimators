...
function [P,Q,delayedP,delayedQ,PpMean,PpStd,QpMean,QpStd,PdiffStd,QdiffStd,Ppsi,Qpsi,Ppsi3,Qpsi3]=loadProcessAssignHalf1ph(Pref,secNodes,dur,resol,delay)
...
allHouses=psize(1)*factor;% total number of household load profile we have
housePmean=zeros(allHouses,psize(2)/dur);%mean
housePstd=zeros(allHouses,psize(2)/dur);%std
houseQmean=zeros(allHouses,psize(2)/dur);%mean
houseQstd=zeros(allHouses,psize(2)/dur);%std
for k=1:allHouses %house index
    for t=1:psize(2)/dur%chosen time period for doing the load assignment according to Pref
...
display('End of load assignment')
